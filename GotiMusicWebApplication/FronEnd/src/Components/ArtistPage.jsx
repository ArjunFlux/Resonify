...
function ArtistPage() {
  ...
  useEffect(() => {
    const fetchSongs = async () => {
      try {
        const res = await fetch("http://localhost:8001/api/show?q=Bollywood");
  ...
  async function handlefilterSong(e) {
    ...
    if (UserChoice == "Bollywood" || UserChoice == "Hollywood") {
      try {
        const ResponseData = await fetch(`http://localhost:8001/api/show?q=${UserChoice}`);
      ...
      try {
        const ResponseData = await fetch(`http://localhost:8001/api/language?q=${UserChoice}`);
        const responseData = await ResponseData.json();
        console.log("User Selected song is : ", responseData);
        setSongsForDisplay(responseData.data.results);
        setisLoading(true);
  ...
  const handleSubmitForArtistName = async (e) => {
    ...
    try {
      const SongResponse = await fetch(`http://localhost:8001/api/song?q=${songname}`);
